...
export class NewsService {
    ...
    static async addComment(postId: string, userId: string, text: string): Promise<boolean> {
        try {
            ...
            console.log('Valid existing comments:', validExistingComments.length, 'out of', post.comments.length);
    ...
    static async updateComment(postId: string, commentId: string, newText: string): Promise<boolean> {
        try {
            ...
            // Update the post in the news database
            const postUpdateSuccess = await this.updatePost(post.slug, { comments: updatedComments });
                ...
                if (allUserContentResponse.ok) {
                    ...
                    if (userContentWithComment) {
                        ...
                        if (!userContentSaveResponse.ok) {
                            console.error('Failed to update user_content:', userContentSaveResponse.status, userContentSaveResponse.statusText);
            ...
            if (userContentResponse.ok) {
                ...
                if (!existingComment) {
                    ...
                    console.log('Available commentIds in user_content:', userContent.comments.map(c => c.commentId));
                ...
                updatedUserComments = userContent.comments.map((comment) => {
                    if (comment.commentId === commentId) {
                        console.log('Updating comment:', comment.commentId, 'from:', comment.text, 'to:', newText);
                ...
                const verifyResponse = await fetch(`${API_BASE_URL}/userContent/get/userId/${commentToUpdate.userId}`);
                if (verifyResponse.ok) {
                    const verifiedUserContent = await verifyResponse.json();
                    const updatedComment = verifiedUserContent.comments.find((c: any) => c.commentId === commentId);
                    console.log('Verification - updated comment text:', updatedComment?.text);
                }
            } else {
                console.error(`User content not found for userId: ${commentToUpdate.userId}`, userContentResponse.status, userContentResponse.statusText);
                return false;
            }
    ...
    static async adminForceDeleteComment(postId: string, commentId: string, adminUserId: string, adminUserName: string): Promise<boolean> {
        try {
            ...
            if (commentToDelete.userId && commentToDelete.userId.trim() !== '') {
                ...
                if (userContentResponse.ok) {
                    ...
                    const updatedAdminComments = [
                        ...(userContent.adminComments || []),
                        newAdminComment
                    ];
    ...
    static async addAdminComment(postId: string, commentId: string, adminCommentText: string, adminUserId: string, adminUserName: string): Promise<boolean> {
        try {
    ...
    static async deleteAdminComment(postId: string, commentId: string, adminCommentId: string): Promise<boolean> {
        try {
    ...
    static async getForceDeletedComments(): Promise<{
        ...
        try {
            ...
            allUserContent.forEach(userContent => {
                const forceDeletedComments = userContent.comments.filter(comment => comment.deletedByAdmin);
